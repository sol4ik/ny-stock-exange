...
```
## Task description
Nowadays it is a quite popular thought that any person can make money on stock exchange - all you need is some starter money and a bit of knowledge.
...
For our final dataset we downloaded the historical data on **Tesla, Inc.** stocks from Yahoo! Finanace from January 5, 2015 until January 4, 2020.  
...
In order to understand the annual return distribution we'll go back to how we calculated it.  
...
#### Conclusions
As we mentioned above, the idea of making money on stocks in very short periods of time is not the best one, since the spread statistics of daily return got very small values and it's just impossible to get a great income, only unless you buy all the shares open on the market.  
Similar thing happens when trying longer time periods like year. Year return showed to have some extreme values with low probability of their occurence and very high probability of getting values close to the expected value.  
In ther words, we can just say that making good money on stocks without any good knowledge involves only a great amount of luck and affecting circumstances. And if you're not lucky or if you simply know something about statistical analysis and probability, you need to use them :)
...
dependence
```
As we can see from the output the greatest correlation here can be seen between the values of low, high, close, open etc. columns. It was quite obvious to get such a result, since they all denote different values of the same, so to say, stock price function, that as we showed before do not change extremely in a short period of time.  
