...
```{r}
# Error here!!!
fit.logis <- fitdistr(tsla.data$YearRet, "normal")
fit.logis
```
```{r}
ks.test(log.ret, "pnorm", mean=mean(log.ret), sd=sd(log.ret))
```

```{r}
ann.rets <- rep(0, nrow(tsla.data))
for (i in 1:nrow(tsla.data)-365) {
  ann.rets[i] <- tsla.data$Close[i + 365] / tsla.data$Close[i] - 1
}
ann.rets

ggplot(data.frame(x = ann.rets), aes(x = x)) +
  geom_histogram()
```
...
There is no sence in dependence between time and the price but there might be some **dependence between prices of the day and the day-before**.  
...
summary(price.lm)
```
p-value of the F-test is almost 0 which means that we **reject $H_0$**. Although residual standart error is quite large to tell that model is good-fitted.   
